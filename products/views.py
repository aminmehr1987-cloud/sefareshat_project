...
def finalize_payment_settlement(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            # ذخیره اطلاعات عملیات برای response (JSON serializable)
            operations_created.append({
                'operation_number': operation_number,
                'operation_id': operation.id,
                'amount': float(payment_amount),
                'customer': customer.get_full_name() if customer else '',
                'description': description
            })
        ...
        # حذف سطر نهایی شده از draft
        # باید draft را پیدا کرده و سطر را از لیست حذف کنیم
        from products.models import InvoiceSettlementDraft
        import jdatetime
        from datetime import datetime
        ...
        if payment_date_str:
            try:
                # تبدیل تاریخ شمسی به میلادی (payment_date_str به فرمت 1404-07-19 است)
                parts = payment_date_str.split('-')
                if len(parts) == 3:
                    j_date = jdatetime.date(int(parts[0]), int(parts[1]), int(parts[2]))
                    work_date = j_date
                else:
                    # اگر فرمت درست نبود، از امروز استفاده کن
                    work_date = jdatetime.date.today()
            except:
                work_date = jdatetime.date.today()
        else:
        ...
        if draft and draft.payments_data:
            # حذف سطر نهایی شده از لیست
            draft.payments_data = [p for p in draft.payments_data if p.get('id') != row_id]
            
            # حذف از دیکشنری‌های JSON field (pop با default value در JSON field کار نمی‌کند)
            if row_id in draft.customer_payments_data:
                del draft.customer_payments_data[row_id]
            if row_id in draft.invoice_settlements_data:
                del draft.invoice_settlements_data[row_id]
        ...
        return JsonResponse({
            'success': True,
            'message': f'عملیات مالی با موفقیت ثبت شد',
